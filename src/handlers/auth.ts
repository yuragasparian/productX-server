...
export async function authUser(req: Request<{}, {}, User>, res: Response) {
  ...
  if (!user.success || !data) {
    ...
    if (!passwordMatches) {
      res.send({
        success: false,
        message: `Wrong password for user @${data.userName}`,
      });
    } else {
      const token = generateJWT({ username: data.useNname, userId: data.id });
      res
        .status(200)
        .json({ success: true, token, message: `Welcome @${data.userName}` });
    }
  }
}
